...
JoinShuffle Signaling Server 2022
...
app.use(
    session({
      secret: process.env.SESSION_SECRET || "6video4Meeting7shuffle",
      resave: false,
...
const api_key_secret = process.env.API_KEY_SECRET || 'JoinShuffle_default_secret';
...
// all start from here
app.get(['/'], (req, res) => {
    var cookieData = req.cookies['curuserinfo'];
    if( req.session.userData == null && cookieData != null ) {
        req.session.userData = cookieData;
        req.session.save();
    }
...
app.get(['/register'], (req, res) => {
    // res.sendFile(path.join(__dirname, 'www/register.pug'));
    if( req.session.userData != null ) {
        if( parseInt(req.session.userData.plantype) == 0 ) 
            res.render('plan.pug', {userinfo: req.session.userData});
        else if( req.session.userData.ispaid == 0 && parseInt(req.session.userData.plantype) == 2 )
            res.render('payment.pug', {userinfo: req.session.userData});
        else if( req.session.userData.roomname == '' ) 
            res.render('roomname.pug', {userinfo: req.session.userData});
        else
            res.render('home.pug', {userinfo: req.session.userData});
...
app.get(['/roomname'], (req, res) => {
    if( req.session.userData != null ) {
        // res.sendFile(path.join(__dirname, 'www/roomname.pug'));
        res.render('roomname.pug', {userinfo: req.session.userData});
    } else {
        // res.sendFile(path.join(__dirname, 'www/landing.html'));
        res.render('login.pug');
...
app.post([apiBasePath + '/meeting'], (req, res) => {
    ...
    if (authorization != api_key_secret) {
        log.debug('JoinShuffle get meeting - Unauthorized', {
            header: req.headers,
            body: req.body,
        });
    ...
    log.debug('Shuffle get meeting - Authorized', {
        header: req.headers,
        body: req.body,
        meeting: meetingURL,
    });
...
io.sockets.on('connect', (socket) => {
    ...
    socket.on('join', (config) => {
        ...
        if( peer_icon == '' ) {
            peer_icon = 'https://eu.ui-avatars.com/api?name=' + peer_name + '&size=140&background=e2fcee&color=00ab53&rounded=false';;
        }
     ...
     * @param {*} channel
     */
     socket.on('requestJoin', (config) => {
        let peer_id = config.peer_id;
        let accept = config.accpet;
        let peers = config.peers;
        let should_create_offer = config.should_create_offer;
        let iceServers = config.iceServers;

        sendToPeer(peer_id, sockets, 'acceptResult', {
            peer_id: peer_id,
            peers: peers,
